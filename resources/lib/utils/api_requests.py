...
def login(ask_credentials=True):
    ...
    try:
        ...
        show_ads_notice = G.LOCAL_DB.get_value('isAdsPlan', None, table=TABLE_SESSION) is None
        if ask_credentials:
        ...
        if is_login_with_credentials:
            ...
            if password and common.make_call('login_auth_data', {'data': data, 'password': password}):
                is_success = True
        if is_success:
            if show_ads_notice and G.LOCAL_DB.get_value('isAdsPlan', False, table=TABLE_SESSION):
                ...
                show_ok_dialog('Netflix - ADS plan',
                               ...
                               'to work properly. Press OK to agree to these terms.')
            return True
